...
public class ExcelToObjectMapper {
    ...
    private void setObjectFieldValueFromCell(Object obj, Field field, Cell cell) {
        ...
        if (cls == String.class) {
            ...
            setNumericValues(cell, field, obj);
        } else if (cls == Long.class) {
            // setNumericValues(cell, field, obj);
        }
        /*
         * else if(cls == Collection.class) { double value =
         * cell.getNumericCellValue(); try { field.set(obj, value); }catch (Exception
         * e) { try { field.set(obj, null); } catch (IllegalAccessException e1) {
         * e1.printStackTrace(); } } }
         */
        else {
    ...
    /**
     * Read first row/header of Excel file, match given header name and return its
     * index.
     * 
     * @param headerName
     * @param workbook
    ...
    private void setNumericValues(Cell cell, Field field, Object obj) {
        if (cell != null && !cell.equals("")) {
            if (cell.getCellTypeEnum() == CellType.STRING) {
                ...
                try {
                    field.set(obj, BigDecimal.valueOf(longValue));
                } catch (Exception e) {
                    try {
                        field.set(obj, null);
                    } catch (IllegalAccessException e1) {
